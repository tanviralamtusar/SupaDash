...
export async function getServerCapacity(signal?: AbortSignal) {
  const { data, error } = await get('/supadash/server/capacity' as any, { signal })
...
export const useServerCapacityQuery = <TData = ServerCapacityData>(
  options: Partial<UseQueryOptions<ServerCapacityData, ResponseError, TData>> = {}
) => {
  ...
  return useQuery<ServerCapacityData, ResponseError, TData>({
    ...
    ...otherOptions,
    queryKey: ['server-capacity', ...(queryKey || [])],
