...
export async function getProjectResources(projectRef: string, signal?: AbortSignal) {
  ...
  const { data, error } = await get(`/supadash/projects/{ref}/resources` as any, {
    params: {
      path: { ref: projectRef },
    },
    signal,
...
export async function getProjectAnalysis(projectRef: string, signal?: AbortSignal) {
  ...
  const { data, error } = await get(`/supadash/projects/{ref}/analysis` as any, {
    params: {
      path: { ref: projectRef },
    },
    signal,
...
export async function updateProjectResources({
  ...
  const { data, error } = await put(`/supadash/projects/{ref}/resources` as any, {
    params: {
      path: { ref: projectRef },
    },
    body: {
